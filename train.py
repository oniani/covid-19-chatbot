...
parser.add_argument(
    "--top_k", type=int, default=40, help="K for top-k sampling."
)
parser.add_argument(
    "--top_p",
    type=float,
...
def main():
    ...
    if args.model_name == "345M" or "774M":
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.val_every > 0:
            val_chunks = (
                load_dataset(enc, args.val_dataset, args.combine)
                if args.val_dataset
                else chunks
            )
        ...
        def validation():
            ...
            v_summary = sess.run(
                val_loss_summary, feed_dict={val_loss: v_val_loss}
            )
