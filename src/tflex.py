...
def latest_checkpoint(checkpoint_dir, latest_filename=None):
    paths = [
        x
        for x in glob(os.path.join(checkpoint_dir, "model-*.*"))
    ...
    if len(ctrs) <= 0:
        ckpt = tf.train.latest_checkpoint(
            checkpoint_dir, latest_filename=latest_filename
        )
        return ckpt
    ctr = ctrs.max()
...
def truncate_value(variable, value, reshape=True):
    ...
    if params2 > params:
        print(
            "Truncating {} from shape {} to shape {}".format(
                variable.name, value.shape, shape
            )
        )
        sys.stdout.flush()
        ...
        print(
            "Expanding {} from shape {} to shape {}".format(
                variable.name, value.shape, shape
            )
        )
        sys.stdout.flush()
...
def assign_values(variables, values, session=None):
    ...
    vals = dict(
        [
            (x.initializer.inputs[1], value)
            for x, value in zip(variables, values)
        ]
    )
...
def load_snapshot(ckpt, session=None, var_list=None, reshape=False):
    ...
    for variables in tqdm.tqdm(list(split_by_params(vs))):
        values = [
            value
            for variable, value in grab_values(
                variables, reader, reshape=reshape
            )
        ]
...
def load_variables(ckpt, session=None, var_list=None, reshape=False):
    ...
    with h5py.File(ckpt, "r") as f:
        for variables in tqdm.tqdm(list(split_by_params(vs))):
            values = [
                truncate_value(x, f[x.name], reshape=reshape)
                for x in variables
            ]
            assign_values(variables, values, session=session)
...
class Saver(object):
    ...
    def restore(self, sess, save_path):
        ...
        elif os.path.exists(save_path + ".npy") or os.path.exists(
            save_path + "-0.npy"
        ):
            load_weights(
                save_path,
...
class Commands(object):
    ...
    def run(self, op):
        ...
        if not ran:
            raise Exception(
                "Commands.execute failed: no such command: {}".format(op)
            )
